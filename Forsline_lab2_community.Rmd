...
Vegetables such as leeks, broccoli, spinach, peas, and carrots contribute very little to PC1 but contribute a moderately to PC2. Similarly, breakfast items such as milk, tea, instant coffee, and muesli contribute little to PC1 but a lot to PC2.
...
-   add fitted surfaces to a plot of fitted vectors with selected
    ...
    -   aluminum is in red
    -   calcium is in green, ranges from 300 - 800, and directs the blue Ca arrow 
    
- `ordisurf()` fits a GAM to the input to create a contour plot, which shows how environmental values vary across sites
...
The ordination surface plot from ordisurf() displays contours of an environmental gradient across sites. It is a more detailed look at an environmental gradient compared to the single blue line vector. 
This environmental overlay is generated by fitting a GAM where the response is the environmental variable of interest and the predictors are a bivariate smooth of the ordination axes, all given by the formula: `Ca ~ s(NMDS1, NMDS2)` 

(Remember each site is associated with a position on the NMDS axes and has an environmental value).  

We can see from the code that the green4 color contours are for Calcium Ca. 
...
The lowest Ca value of 350 is located in the lower left of the area while the highest value of 700 is located near the upper right corner. In other words, the lowest Ca value is found at relatively lower values of NMDS1 and NMDS2 while the highest CA value is found at higher values of NMDS1 and NMDS2. 
...
Sites 28 and 4 are most differentiated by CCA1. Aluminum is the strongest environmental vector for CCA1. 
